...
// Creates the interface for the TensorFlow Op 'cubic_interpolation2d'.
REGISTER_OP("CubicInterpolation2D")
    .Input("input: float")
    .Output("output: float")
    .Attr("factors: list(int) >= 2")
    .Attr("output_spatial_shape: list(int) >= 2")
    .SetShapeFn([](InferenceContext* c) {
      ...
      c->set_output(0, c->MakeShape({out_shape_0, out_shape_1, channels}));
